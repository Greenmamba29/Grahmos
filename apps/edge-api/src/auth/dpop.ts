 ...
 */
const generateJwkThumbprint = (jwk: any): string => {
  // Create canonical JWK representation for thumbprint
  const canonicalJwk: any = {};
 ...
 */
const validateDPoPProof = (dpopToken: string, method: string, uri: string): { valid: boolean; jkt?: string; error?: string } => {
  try {
 ...
 */
export function requireDPoP(req: Request, res: Response, next: NextFunction) {
  ...
  if (dpopHeader) {
    ...
    (req as any).dpopThumb = validation.jkt;
  }
 ...
 */
export function requirePoPJWT(req: Request, res: Response, next: NextFunction) {
  ...
  try {
    ...
    (req as any).jwtClaims = claims;
    (req as any).authMethod = isMtls ? 'mtls' : 'dpop';
