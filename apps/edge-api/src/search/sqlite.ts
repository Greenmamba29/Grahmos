...
export class SqliteSearchBackend implements SearchBackend {
  ...
  async search(query: string, options: SearchOptions = {}): Promise<SearchResult[]> {
    ...
    try {
      ...
      const rows = stmt.all(ftsQuery, limit, offset) as any[];
      ...
      try {
        ...
        const rows = stmt.all(likeQuery, likeQuery, limit, offset) as any[];
  ...
  async getStatus(): Promise<BackendStatus> {
    ...
    try {
      ...
      return {
        healthy: true,
        version: this.db.pragma('user_version', { simple: true }) as string,
