...
interface ReceiptPayload {
  orderId: string
  ts: number
  // additional fields may exist but are not used here
  [key: string]: unknown
}

export async function enqueuePurchase(payload: PurchasePayload){
...
export async function flushPurchases(pubkeyB64:string){
  ...
  for (const it of intents){
    try {
      ...
      const { receipt, sig }: { receipt: ReceiptPayload; sig: string } = await res.json()
      ...
      await db.purchaseQueue.update(it.id, { status: 'queued', lastError: message })
    }
  }
}
