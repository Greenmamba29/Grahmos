...
class AssistantTelemetry {
     ...
     */
    recordLLMRequest(model, duration, success, tokenCount, errorType) {
     ...
     */
    recordTTSRequest(engine, duration, success, audioLength, errorType) {
        if (!this.isInitialized)
            return;
        const attributes = {
            'tts.engine': engine,
     ...
     */
    trackSessionStart(sessionId, attributes = {}) {
        if (!this.isInitialized)
            return;
    ...
    trackSessionEnd(sessionId, duration, messageCount) {
        if (!this.isInitialized)
            return;
        this.activeSessionsGauge?.add(-1, {
            'session.id': this.hashSessionId(sessionId),
    ...
    startSpan(name, attributes = {}, parentSpan) {
        ...
        if (parentSpan) {
            span.setParent && span.setParent(parentSpan);
        }
        return span;
    }
    /**
     ...
     * Record an error
     */
    recordError(error, component, severity = 'medium', attributes = {}) {
        if (!this.isInitialized)
            return;
        this.errorCounter?.add(1, {
