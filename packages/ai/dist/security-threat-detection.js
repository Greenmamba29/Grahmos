...
class IsolationTree {
    ...
    getPathLength(point, node, currentDepth) {
        if (node.isLeaf()) {
...
class AutoencoderAnomalyDetector {
    ...
    inputShape;
    threshold = 0.95;
    constructor(inputShape, hiddenLayers = [32, 16, 8, 16, 32]) {
        this.inputShape = inputShape;
        this.buildModel(hiddenLayers);
    }
    buildModel(hiddenLayers) {
...
export class SecurityThreatDetectionSystem {
    ...
    constructor(config = {}) {
        ...
        this.isolationForest = new IsolationForest(this.config.models.anomalyDetection.isolationForest.nTrees, this.config.models.anomalyDetection.isolationForest.maxSamples);
        if (this.config.models.anomalyDetection.autoencoder.enabled) {
            this.autoencoder = new AutoencoderAnomalyDetector(this.config.features.network.length + this.config.features.behavior.length, this.config.models.anomalyDetection.autoencoder.hiddenLayers);
        }
    }
    /**
     * Train the ML models with historical data
     */
    async train(historicalEvents) {
    ...
    async analyzeEvent(event) {
        ...
        const autoActions = this.config.autoResponse.enabled
            ? this.config.autoResponse.actions[threatLevel.toLowerCase()]
            : undefined;
        const result = {
            eventId: event.id,
            threatLevel,
    ...
    extractFeatures(event) {
        const features = [];
        // Network features
        features.push(this.normalizeValue(event.responseTime, 0, 10000), // Response time
        ...
        features.push(this.calculateUserAgentRisk(event), // User agent risk
     ...
     */
    buildOrUpdateUserProfile(event) {
        if (!event.userId) {
            return {
                typicalHours: [],
                commonLocations: [],
     ...
     * Calculate overall threat score
     */
    calculateThreatScore(anomalies, behaviorAnalysis, threats) {
        let score = 0;
     ...
     */
    generateRecommendations(threats, anomalies, behaviorAnalysis) {
     ...
     */
    async executeAutoActions(event, result, actions) {
    ...
    calculateRequestRate(event) {
        const timeWindow = 60000; // 1 minute
        const recentEvents = this.eventHistory.filter(e => e.ip === event.ip &&
            event.timestamp - e.timestamp < timeWindow);
        return recentEvents.length / 60; // requests per second
    }
    calculateGeoRisk(event) {
    ...
    calculateLocationRisk(event, userProfile) {
        if (!event.geoLocation || !userProfile)
            return 0.5;
    ...
    detectBruteForce(event) {
        ...
        const recentFailures = this.eventHistory.filter(e => e.ip === event.ip &&
            e.statusCode === 401 &&
            event.timestamp - e.timestamp < timeWindow);
    ...
    detectDDoS(event) {
        const timeWindow = 60000; // 1 minute
        const recentRequests = this.eventHistory.filter(e => e.ip === event.ip &&
            event.timestamp - e.timestamp < timeWindow);
    ...
    analyzeUserEvents(userId, events) {
    ...
    calculateBaselineRiskScore(events) {
        ...
        if (uniqueUserAgents > 2)
            risk += 0.1;
        return Math.min(risk, 1);
    }
    calculateUserRiskScore(event, profile) {
    ...
    analyzeSession(event) {
        ...
        const suspiciousPatterns = [];
        if (errorRate > 0.5)
            suspiciousPatterns.push('high_error_rate');
        if (uniqueEndpoints > 20)
            suspiciousPatterns.push('endpoint_scanning');
        if (duration > 3600000)
            suspiciousPatterns.push('unusually_long_session'); // > 1 hour
        return {
    ...
    calculateRiskFactors(event, profile) {
        const factors = [];
        // Time-based risk
        const hour = new Date(event.timestamp).getHours();
        if (hour < 6 || hour > 22) {
    ...
    async alertSecurity(event, result) {
...
export class RealTimeThreatMonitor {
    ...
    constructor(detectionSystem) {
        this.detectionSystem = detectionSystem;
    }
    start(processingIntervalMs = 1000) {
