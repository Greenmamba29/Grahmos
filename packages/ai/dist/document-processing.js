...
(function (DocumentCategory) {
    ...
    DocumentCategory["EMAIL"] = "EMAIL";
    DocumentCategory["MANUAL"] = "MANUAL";
    DocumentCategory["POLICY"] = "POLICY";
    DocumentCategory["RESEARCH"] = "RESEARCH";
    DocumentCategory["NEWS"] = "NEWS";
...
export class ContentExtractionService {
    ...
    async extractFromText(textBuffer) {
...
export class EntityRecognitionService {
    ...
    async extractWithOpenAI(text, types) {
...
`;
    ...
    getContext(text, startIndex, length, contextSize = 50) {
    ...
    mergeEntities(entities) {
        ...
        for (const entity of sorted) {
            const overlapping = merged.find(existing => entity.startIndex < existing.endIndex && entity.endIndex > existing.startIndex);
            if (!overlapping) {
                merged.push(entity);
            }
...
export class DocumentCategorizationService {
    ...
    async categorizeDocument(text, title) {
        try {
    ...
    categorizeWithRules(text, title) {
    ...
    async categorizeWithAI(text, title) {
...
export class AutoTaggingService {
    ...
    async generateTags(text, title, maxTags = 10) {
        try {
    ...
    async generateAITags(text, title, maxTags) {
        const prompt = `
...
`;
    ...
    combineTags(keywordTags, aiTags) {
        ...
        const filteredTags = uniqueTags.filter(tag => tag.length >= 3 && tag.length <= 30 && !/^\d+$/.test(tag));
    ...
    scoreTag(tag, keywordTags, aiTags) {
        let score = 0;
...
export class DocumentProcessingService {
    ...
    completeProcessingStep(step, result) {
    ...
    detectLanguage(text) {
        ...
        if (/[äöüß]/.test(sample))
            return 'de'; // German
        if (/[аеиоуыэюя]/.test(sample))
            return 'ru'; // Russian
        if (/[一-龯]/.test(sample))
            return 'zh'; // Chinese
        if (/[ひらがなカタカナ]/.test(sample))
            return 'ja'; // Japanese
    ...
    assessQuality(text, metadata) {
        ...
        if (avgSentenceLength > 10 && avgSentenceLength < 30)
            textQuality += 0.1;
        if (text.match(/[A-Z]/g) && text.match(/[a-z]/g))
            textQuality += 0.1;
        ...
        if (text.match(/^\s*-|\*|1\./m))
            structureQuality += 0.2; // Has lists
        if (text.match(/^#{1,6}\s/m))
            structureQuality += 0.3; // Has headings
...
// Factory function
export function createDocumentProcessingService(openaiApiKey) {
