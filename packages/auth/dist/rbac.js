...
export class RBACService {
    ...
    evaluateCondition(condition, context) {
        ...
        if (condition.field.startsWith('user.')) {
            ...
            fieldValue = context.environment && context.environment[envKey];
        }
        else {
    ...
    async canRead(userId, resource, context) {
        const result = await this.checkAccess({ userId, resource, action: 'read', context });
        return result.granted;
    }
    async canWrite(userId, resource, context) {
    ...
    async canCreate(userId, resource, context) {
    ...
    async getEffectivePermissions(userId, context) {
        const roles = this.getUserRoles(userId);
...
export function createRBACMiddleware(rbacService) {
    return {
        ...
        requireRole: (...roleIds) => {
            return (req, res, next) => {
                ...
                const hasRole = roleIds.some(roleId => userRoles.some(role => role.id === roleId));
                if (!hasRole) {
                    return res.status(403).json({
