...
export class AuthenticationService {
    ...
    async verifyToken(token, audience = 'grahmos-api') {
        const options = {
            algorithms: ['RS256'],
            issuer: 'grahmos-auth',
            audience,
    ...
    generateMfaSecret() {
        ...
        const qrCodeUrl = authenticator.keyuri('user@example.com', // This would be the actual user email
        this.config.mfaServiceName, secret);
    ...
    async createSession(user, request) {
        ...
        if (this.redis) {
            await this.redis.setex(`session:${session.id}`, this.parseExpiry(this.config.sessionExpiry) / 1000, JSON.stringify(session));
        }
        return session;
    }
    ...
    // Audit logging
    async auditLog(action, userId, details, ipAddress) {
    ...
    async login(request) {
        ...
        if (user.mfaEnabled) {
            if (!mfaCode) {
                return { user, tokens: {}, requiresMfa: true };
            }
            // Verify MFA code
    ...
    async disableMfa(userId, mfaCode) {
        const user = await this.getUserById(userId);
        if (!user) {
            throw new Error('User not found');
        }
